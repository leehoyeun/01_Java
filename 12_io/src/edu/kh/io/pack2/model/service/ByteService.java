...
public class ByteService {
	 ...
	 * 
	 * Byte 기반 스트림 - 1byte 단위로 데이터를 입/출력하는 스트림 - 최상위 클래스 : InputStream ,
	 * OutputStream - 문자열 ,이미지, 영상,오디오,pdf 등 모든것을 입/출력 할 수 있음 (단,통로가 좁다보니 속도가 좀 느림,
	 * char 형 문자 같은 경우 깨질 우려가 있음)
	 * 
	 * 
	 */

	// IO 관련된 코드 IOEception이 발생될 가능성이 높다!
	...
	public void fileByteOutput() {
		...
		try {
			...
			sb.append("~!@#$%^&*)_+\n");
			sb.append("가나다라마바사아\n");
			sb.append("와 자바 너무 재밋다\n");
			sb.append("선생님 저 공부 열심히 할게요\n");
			sb.append("진짜로!!정말로!!화이팅!!");

			// 출력방법 1 : 한글자씩(2byte)씩 파일로 출력
			...
			// FileOutputStream 에서는 flush() 선택 사항
			...
			try {// 스트림이 정상 생성된 경우
					...
					// close()메서드도 IOException 발생시키기 때문에
	...
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * ->보조스트림 이용 시 버퍼에 입출력할 내용을 모아서 한번에 입출력 (내부 버퍼에 데이터를 모아뒀다가 일정 크기가 되면 한번에 출력)
		...
		try {
			...
			// 기반스트림 fos 를 이용해서 보조스트림 bos를 생성
	...
	public void fileByteInput() {
		...
		try {
			...
			while (true) {
				...
				if (value == -1)
					break; // 다읽어왔으면 반복 멈춤
				sb.append((char) value); // char로 강제 형변환해서 글자 형태로 sb에 추가
	...
	public void bufferedFileByteInput() {
		...
		try {
			...
			try {
				//보조 스트림만 close()해도 기반 스트림도 자동 종료된다
	 ...
	 * -> Map 계열 Properties 클래스 사용하기
	 * 
	 */
	public void readProperties() {
		
		// Properties 클래스는 Hashtable의 하위클래스로 Map계열 클래스 중 하나.
		// Key 와 Value 모두 String 타입으로 이루어짐.
		...
		try {
			...
			//properties.getProperty(Key) : Key 에 대응되는 value 값 반환
	 ...
	 * 
	 * [실행화면]
	 * 파일경로 입력 : /io_test_20250305/노래가사.txt
	 ...
	 * -> BufferedReader 생성 시 기반스트림인 InputStreamReader 사용 -> InputStream가 기반
	 * 
	 */
	public void fileCopy() {
		...
		try {
			...
			br = new BufferedReader(new InputStreamReader(System.in));
						//InputStreamReader 문자스트림 변환	//System.in 바이트 스트림
			// 경로 입력 받기
