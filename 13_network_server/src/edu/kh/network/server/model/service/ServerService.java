...
public class ServerService {
	...
	/*
	 * TCP - 서버, 클라이언트간 1:1 소켓통신 (연결 지향적 프로토콜) 
	 ...
	 * - 클라이언트의 연결 요청이 들어오면 클라이언트의 소켓을 얻어와
	 * 연결하게 된다.
	 *
...
//	4).	접속 요청이 오면 요청 수락 후 해당 클라이언트에 대한 소켓 객체 생성
		...
		// -> 수락 시 클라이언트의 socker 전달 받음
		
		System.out.println("[클라이언트 접속 성공]");
//	5).	연결된 클라이언트와 입출력 스트림 생성
		// -> 클라이언트가 서버 요청 시 소켓을 전달
		// ->소켓에 입/출력 스트림이 담겨있음
		is = clientSoket.getInputStream(); //입력용 스트림
...
//	7).	스트림을 통해 읽고 쓰기
		...
		//						new Date()객체 생성시 현재 시간이 저장이 되어있다.
		
		//SimpleDateFormat : 날짜 형식을 간다히 지정하는 객체
		...
		String clientIp = clientSoket.getInetAddress().getHostAddress();
		...
		try {
			...
			if(clientSoket != null)clientSoket.close();
