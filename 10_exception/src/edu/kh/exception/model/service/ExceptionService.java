...
public class ExceptionService {
	...
	public void ex2() {
		...
		try {
			...
			//InputMismatchException,ArithmeticException 둘다 런타임이섹션 후손
		...
		//RuntimeException 부조 객체가 먼저 전부 처리하여서 뒤에 작성 
		
		}catch(Exception e) {
			// Exception 클래스 : 모든 예외의 최상위 부모
			// 다형성 -업캐스팅 : 부모타임 참조변수로 자식객체 참조
	...
	public void ex3() {
		...
		try {
			...
			e.printStackTrace();//종합적으로 어느위치에서 발생한것인지 보여주는거
		}catch(InputMismatchException e) {
			System.out.println("무조건 줏자만 넣어라");
		}		
		finally {
