...
public class VariableExample2 {
	...
	public static void main(String[] args) {
		...
		// 변수 언언 : 메모리(RAM)에 값을 저장할 공간을 할당하는 것.
		// 변수 값 대입: 변수에 값을 집어 넣는 것.
		
		boolean booleanData ; // 변수의 선언 
		// 메모리에 논리값 (t/f) 저장할 공간을 1byte 할당하고
		// 할당된 공간의 이름을 booleanData 라고 저장하겠다.
		...
		byte byteNumber = 127; //128을 넣는 순간 에러 ( byte 값의범위 -128 ~ 127)
		//메모리에 정수값을 공간을 1byte 할당하고
		//할당된 공간을 byteNumber 라고 부르겠다.
		// 선언된 byteNumber 변수에 처음으로(초기값)127 넣음
		// 초기화 : 처음 변수에 값을 대입 
		
		
		short shortNumber = 32767;
		
		//정수 자료형 기본형 ! int ( short, byte 는 거의안씀 옛날코드 잔재 )
	...
	// 자료형 변수명 대입연산자 리터럴;
		...
		// The literal 1000000000 of type int is out of fange
		// 100억이라는 값은 int 의 범위를 벗어났다
		//리터럴값 자체를 int로 인식하고있다.
		//뒤에 L 을 붙여줌으로써 long 자료형인것을 나타내줌.
		
		
		float floatNumber = 1.2345f;//F또는 f
		 ...
		 * -컴퓨터에는 문자표가 존재하고 있음
		 * 숫자에 따라 지정된 문자 모양이 각각 매핑이되고
		 * 'B' 문자 그대로 대입되면 변수에 숫자 66으로 변환되어 저장
		 * →반대로 생각하면 변수에 애초에 66이라는 숫자를 저장하는것도 가능함.
		 * (유니코드 문자책에 저장되어있는 문자를 불러옴) 
		 * */
		
		
		//변수 명명 규칙
		// 1. 대소문자 구분 , 길이제한 X
		int abcdefG123456789;//G 가 다름
		...
		//4. 특수문자 $ , _ 만 사용 가능 (사용은 가능하나 쓰지 않는다.)
		int $intNumber;//문제 없음
		...
		//6. 변수명은 언어를 가리지 않는다(한글도 가능하나 쓰지 않는다.)
		int 정수1번;
		double 실수2번 = 3.14;
		 ...
		 * 상수 명명 규칙 : 모두 대문자 ,어려단어 작성 시 _ 사용 
		 * 
		 * 상수를 사용하는 경우
